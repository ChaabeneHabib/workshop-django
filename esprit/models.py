...
def is_esprit_mail(email):
    if str(email).endswith("@esprit.tn")==False:
        raise ValidationError("votre mail doit contenir @esprit.tn",params={'value':email})
...
class Project(models.Model):
    ...
    def total_allocated_by_members(self):
        list_members_In_Project = MembershipInProject.objects.filter(projet=self.pk)
        sum_by_members = list_members_In_Project.all().aggregate(Sum('time_allocated_by_member'))
